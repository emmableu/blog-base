---
title: 动态规划 - Dynamic Programming - DP - 模版
date: 2022-07-27 09:57:26
permalink: /pages/d7dd7a/
categories: 
  - Google
  - 基本模版
tags: 
...
### best sightseeing pair 类似题
- [344 - 1014 - Best Sightseeing Pair - Medium](/pages/4d2d7f/)
- [1937 - Maximum Number of Points with Cost](/pages/f18bde)
...
根据伪码，我们加上 base case 即可得到最终的答案: 显然目标金额为 0 时，所需硬币数量为 0


![](https://raw.githubusercontent.com/emmableu/image/master/202209111235863.png)
...
class Solution:
    def coinChange(self, coins, amount):
        change = float('inf') # if amount < 0
        if amount == 0:
            change = 0
        elif amount > 0:
            for coin in coins:
                change = min(change, self.coinChange(coins, amount - coin) + 1)
        return change if change < float('inf') else -1 # 不能在这里return，这样的话就不能层层返回inf，在最后处理了
```
...
class Solution:
    ...
    def dfs(self, coins, amount):
        ...
        if amount == 0:
            ...
            for coin in coins:
                change = min(change, self.dfs(coins, amount - coin) + 1)
        return change
```
...
[![](https://labuladong.github.io/algo/images/%e5%8a%a8%e6%80%81%e8%a7%84%e5%88%92%e8%af%a6%e8%a7%a3%e8%bf%9b%e9%98%b6/coin.png)](https://labuladong.github.io/algo/images/%e5%8a%a8%e6%80%81%e8%a7%84%e5%88%92%e8%af%a6%e8%a7%a3%e8%bf%9b%e9%98%b6/coin.png)

至此，这个问题其实就解决了，只不过需要消除一下重叠子问题，比如 `amount = 11, coins = {1,2,5}` 时画出递归树看看：

[![](https://labuladong.github.io/algo/images/%e5%8a%a8%e6%80%81%e8%a7%84%e5%88%92%e8%af%a6%e8%a7%a3%e8%bf%9b%e9%98%b6/5.jpg)](https://labuladong.github.io/algo/images/%e5%8a%a8%e6%80%81%e8%a7%84%e5%88%92%e8%af%a6%e8%a7%a3%e8%bf%9b%e9%98%b6/5.jpg)
...
class Solution:
    ...
    def dfs(self, coins, amount, memo):
        if memo[amount] != -999: # 这里错了，因为没有考虑amount<0的情况
            return memo[amount]
        change = float('inf') # if amount < 0
        if amount == 0:
            change = 0
