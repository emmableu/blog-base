...
然后给一个mapping，k是字母，v是bit length（注意是length）：
`{b: 2, a: 1, c: 2}`

题目是给定这个mapping，还原那个tree。可以assume：
1. tree is always valid
2. if length is same, smaller value is always on left (example: b is on left to c in the tree)
3. shorter path is always on the left (我也没太懂，比如a的length小，所以是shorter path，会在左边）
...
### shortest sequence of ball movements
